...
def playerChoice(player: Character, enemies: list[Character]):
    ...
    match key:
        ...
        # defend
        case "D":
            print("\r" + " " * 100 + "\r", end='', flush=True)  # Clear the line before printing the action
        ...
        case "S":
            ...
            if len(enemies) > 1:
                ...
                damage_to_enemy = max(0, player_special_attack - enemyToSpecialAttack.defense)
                enemyToSpecialAttack.base_health -= damage_to_enemy
                print(f"You special attack {enemyToSpecialAttack.name} for {player_special_attack} damage. {enemyToSpecialAttack.name} defends for {enemyToSpecialAttack.defense} damage, taking {damage_to_enemy} damage.")
...
def enemyChoice(player: Character, enemyToAct: Character):
    ...
    match choice:
        ...
        case "special attack":
            ...
            player.base_health -= damage
            print(f"{enemyToAct.name} attacks you for {enemy_special_attack} damage. You defend for {player.defense} damage, taking {damage} damage.")
